...
function newsMatchesQuery(n: NewsItem, q: string): boolean {
  if (!q) return true;
  const haystack = [
    n.id,
    n.headline,
...
function NewsView() {
  ...
  // Sync URL on query / hideSeen changes. Chip filter state is intentionally
  // transient (it's a quick-flip, not a deep-link dimension).
  useEffect(() => {
  ...
  const filtered = useMemo(() => {
    return news.filter((n) => {
      ...
      if (!newsMatchesQuery(n, query)) return false;
      if (hideSeen && seenHydrated && seenIds.includes(n.id)) return false;
      return true;
    });
  }, [filter, query, hideSeen, seenHydrated, seenIds]);

  const seenCount = seenHydrated
    ? news.filter((n) => seenIds.includes(n.id)).length
    : 0;
  ...
  return (
    <div className="bg-grid-fine">
      ...
      <div className="mx-auto max-w-7xl px-6 py-10">
        ...
        {/* Seen state controls — only visible when there's something to act on */}
            ...
            <div className="mt-6 flex flex-wrap justify-center gap-2">
              {filter !== "all" && query && (
                <button
                  ...
                  className="border border-amber bg-amber/10 px-4 py-2 font-mono text-[10px] uppercase tracking-widest2 text-amber hover:bg-amber hover:text-ink"
                >
                  ↶ Reset all
                </button>
              )}
              {filter !== "all" && !query && (
                <button
                  ...
                  className="border border-amber/60 px-4 py-2 font-mono text-[10px] uppercase tracking-widest2 text-amber hover:bg-amber hover:text-ink"
                >
                  ← Reset filter
                </button>
              )}
              ...
              <div className="col-span-12 lg:col-span-2">
                <div className="flex items-center gap-2">
                  <div className="font-mono text-[10px] uppercase tracking-widest2 text-amber">
                    <Highlight text={n.id} query={query} />
                  </div>
                  {isSeen && (
