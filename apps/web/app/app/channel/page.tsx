          ...
          <div className="flex items-baseline justify-between">
            <div>
              ...
              <h1 className="mt-2 font-display text-5xl text-paper">
                My <span className="italic text-amber">Channel</span>
              </h1>
      ...
      <div className="mx-auto max-w-3xl px-6 py-10">
        ...
        <div className="mt-6 space-y-10">
          {channelPosts.map((p, i) => (
            <article
              key={p.id}
              className="relative border-l-2 border-amber/40 pl-6"
            >
              <div className="absolute -left-1.5 top-1.5 h-3 w-3 bg-amber" />
              <div className="flex flex-wrap items-baseline gap-3 font-mono text-[10px] uppercase tracking-widest2">
                <span className="text-amber">{p.id}</span>
                <span className="text-paper/40">·</span>
                <span className="text-paper/60">
                  {formatDate(p.ts)} · {formatTime(p.ts)}Z
                </span>
                <span className="text-paper/40">·</span>
                <span className="text-amber">BY {p.author.toUpperCase()}</span>
              </div>
                ...
                <div className="mt-3 flex flex-wrap gap-2">
                  {p.tags.map((t) => (
                    <Link
                      key={t}
                      href={`/app/tags/${t}`}
                      className="font-mono text-[10px] uppercase tracking-widest2 text-amber/70 hover:text-amber"
                    >
                      #{t}
                    </Link>
                  ))}
