...
- [ ] **[reusability] Three hooks duplicate the same localStorage-backed string-set pattern** — `useSeenNews`, `useReadPrimers`, `useWatchlist` all implement: `load()` / `save()` / JSON.parse with array-of-string filter, `useState + useEffect hydration + CustomEvent cross-instance sync`. Extract a generic `usePersistedStringSet(key, eventName)` into `apps/web/lib/hooks/` and rewrite the three hooks as thin wrappers. ~60 LOC removed, one place to fix future bugs (storage-quota handling, SSR guard). _(found tick 106, apps/web/features/news/hooks/useSeenNews.ts + education/useReadPrimers.ts + watchlist/useWatchlist.ts)_
