...
- [ ] **D5.** More macro sources — FOMC / CPI ingested into `timeline_events`. **Unblocked** as of tick 127 (ForexFactory adapter landed under Section 3 Calendar; `source_code='FF'` mirrors to `kind='macro'`). The generic calendar pipeline is ready; individual FOMC / CPI wirings (as curated/promoted events, or per-indicator filters on FF) can now proceed in a future tick.
