...
- [ ] **[performance] EventFeed re-renders every card on each realtime INSERT** — `EventCard` is not wrapped in `React.memo`, and `EventFeed.map` passes a fresh object ref each render. With the 500-row cap in `useTimelineEvents`, a single realtime INSERT re-renders up to 500 cards. Memoize `EventCard` keyed by `event.id` to cut re-renders to O(1) per INSERT. _(found tick 102, apps/web/features/timeline/components/EventCard.tsx:29)_
