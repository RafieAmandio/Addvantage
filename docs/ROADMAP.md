...
- [ ] **Rate limit per tier** — carved into sub-items tick 167. Today `lib/ratelimit.ts` applies a flat per-user bucket regardless of `profiles.tier` (free vs vip). This epic differentiates budgets by tier so VIP gets room to actually use the desk while free stays behind a cheap cap. L4's token-accounting metadata (tick 154) lets us layer a daily token budget on top of the existing per-minute rate.
  - [ ] **RT1.** Tier-aware limiter helper — new `apps/web/lib/ratelimit-tier.ts` wrapping `rateLimit({ key, limit, windowSec })` with a `(userId, tier, action)` signature. Per-tier bucket table (free = restrictive, vip = generous), resolved at call time. Graceful no-op when Upstash unset (inherits the existing lib contract). Single place to tweak tier budgets going forward. Exports both a named `enforceTierRateLimit()` helper and a `TIER_BUCKETS` const for test visibility.
