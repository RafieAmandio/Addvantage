...
- [ ] **[reusability] SymbolNav + IntervalPicker are structurally identical** — both in `features/chart/components/` render the same segmented-pill nav (Link with `border-lime bg-lime text-ink` active state) over a `readonly string[]`. Extract a generic `SegmentedNav<T extends string>` primitive into `components/ui/` (two consumers in one feature is borderline, but a third callsite is likely as soon as watchlist or plan filters need the same control). _(found tick 94, apps/web/features/chart/components/SymbolNav.tsx:15 + IntervalPicker.tsx:22)_
