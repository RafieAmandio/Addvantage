...
- [ ] **Setup outcome tracking (win/loss/R) tied to market close prices** — carved into sub-items tick 160. The `trading_plans` table (migration 0021) already carries `outcome` + `closed_at`; the admin `closePlan` action (tick 148) already sets outcome. What's missing: (a) the *price* the plan was closed at and the realized-R derived from it, (b) user-facing stats (win rate, avg R) on `/app/plan`, (c) optional automation that detects target/stop hits.
  ...
  - [ ] **O3.** Stats surface — new `features/plan/queries/stats.ts` `getClosedPlanStats({ limit? })` returning `{ n, winRate, avgR, byDirection }`. Render a `PlanStatsBadges` component (feature-local) on `/app/plan` top-of-page. Stats are derived from `status='closed'` rows with non-null `realized_r`. Zod-validated.
