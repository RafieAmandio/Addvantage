...
- [ ] **B1.** Provider-agnostic `BarsAdapter` interface (`apps/worker/src/adapters/bars/base.ts`) + `TwelveDataAdapter` implementation + worker config envs `MARKET_DATA_PROVIDER` and `MARKET_DATA_API_KEY`. Registry in `apps/worker/src/adapters/bars/index.ts`.
- [ ] **B2.** Worker one-shot CLI `pnpm --filter @tradevantage/worker run:bars <SYMBOL>` that fetches via the adapter and upserts into `instrument_bars`.
...
- [ ] **[code-quality] Oversized route files (>400 LOC) — remaining** — `/app/page.tsx` (774), `/app/plan/archive/page.tsx` (724), `/app/watchlist/page.tsx` (573), `/page.tsx` (521), `/app/plan/compare/page.tsx` (494). Apply the same pattern as consult + calendar: lift presentational components into `features/<name>/components/`, pure helpers into `features/<name>/lib/`, types into `features/<name>/types.ts`. _(found tick 28, carved out tick 31)_
