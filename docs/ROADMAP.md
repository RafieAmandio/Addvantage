...
- [ ] **[code-quality] LiveEventFeed heading count goes stale as new events stream in** — `/app/chart/[symbol]/page.tsx` passes `heading={`Timeline · ${events.length} events`}` where `events.length` is the server-rendered initial count. After D3 realtime appends new rows client-side, the heading keeps showing the old number. Move the count inside `LiveEventFeed` (compute from `events` state) or accept a `renderHeading(count)` prop so the displayed number tracks reality. _(found tick 91, apps/web/app/app/chart/[symbol]/page.tsx:136)_
- [ ] **[observability] Admin `saveEdits` swallows DB error without logging** — returns `{ ok: false, error: error.message }` to the client but never hits `logger.error`, so intermittent RLS/constraint failures leave no server-side trace. Add a `logger.error("saveEdits failed", { id, error, scope: "admin.saveEdits" })` before the return; sibling `approveItem`/`rejectItem` already throw so they surface, this one is silent. _(found tick 90, apps/web/app/admin/review/[id]/actions.ts:72)_
