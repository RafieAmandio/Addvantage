...
- [ ] **[performance] Multiple permissive RLS policies overlap on news_items / profiles / sources / timeline_events** — Supabase advisor `multiple_permissive_policies`. Current overlap snapshot (per role × action, roles = {anon, authenticated, authenticator, dashboard_user, supabase_privileged_role} — all 5 roles share identical overlaps because every policy targets `public`): `news_items` SELECT = 3 policies (`news_items_admin_read`, `news_items_admin_write` FOR ALL, `news_items_public_read`); `profiles` UPDATE = 2 (`profiles_admin_update`, `profiles_self_update`); `sources` SELECT = 2 (`sources_admin_write` FOR ALL, `sources_public_read`); `timeline_events` SELECT = 3 (`timeline_events: admin read all`, `timeline_events: owner read user_pin`, `timeline_events: public read editorial`). Audit pass done tick 60 — draft migration at `packages/db/migrations/_proposed_0013_consolidate_rls.sql` (leading underscore keeps it inert; Supabase `list_migrations` reads `supabase_migrations.schema_migrations`, not the directory). Remaining steps for a future tick: rename `_proposed_0013_consolidate_rls.sql` → `0013_consolidate_rls.sql`; apply via `mcp__supabase__apply_migration`; regenerate types (`supabase gen types typescript ... > packages/db/src/types.ts`); run `mcp__supabase__get_advisors` performance; smoke-test admin + public reads on `/app/news`, `/admin/review`, and timeline pins. Security-sensitive — diff the before/after policy set before applying. _(found tick 27, audit tick 60, packages/db/migrations/0003_rls.sql + 0007_timeline.sql)_
...
- [ ] **[code-quality] Oversized feature components (>400 LOC)** — distinct from the route-files item: `PlanDetail.tsx` 631 → 160 done tick 60 (extracted `PlanDetailHeader` / `PlanDetailOutcomeSummary` / `PlanDetailSetupCard` / `PlanDetailInformingNews` / `PlanDetailRisks` + pure helpers `setupToText` / `planToMarkdown` / `OUTCOME_META` into `features/plan/lib/detail-helpers.ts`); remaining: `features/search/components/SearchPalette.tsx` (503), `features/calendar/components/CalendarPageView.tsx` (489), `features/consult/components/ConsultPageView.tsx` (468), `features/notifications/components/NotificationBell.tsx` (456). Same pattern as the route splits — extract sub-components and pure helpers within the feature folder. Lower priority than route splits since these don't bloat the page-level bundle. _(found tick 59, scan category 0; PlanDetail done tick 60)_
