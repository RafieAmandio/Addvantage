...
- [ ] **Event → news correlation** — carved into sub-items tick 172. Today `timeline_events` (macro kind, from FF adapter) and `news_items` live side-by-side but have no link, so a user looking at a CPI release on the calendar can't see which news items covered it. Goal: surface the set of related news for any given calendar event, without hand-curation.
  ...
  - [ ] **EN2.** Surface on calendar detail — extend `/app/calendar/[id]` (or the event detail drawer/modal if that's the current shape) with a "Related news" section rendering a `RelatedNewsList` feature-local component (`features/calendar/components/`). Empty-state when none match. Link each row to `/app/news/[id]`.
