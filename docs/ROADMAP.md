...
- [ ] **[dx] turbo `typecheck` task wastefully depends on `^build`** — `turbo.json` declares `"typecheck": { "dependsOn": ["^build"] }` but `@tradevantage/db` and `@tradevantage/shared` ship raw `.ts` from `src/` (per CLAUDE.md "no build step"). The `^build` dependency forces an upstream `build` before any typecheck — pure waste on cold cache. Drop `dependsOn` (or set to `["^typecheck"]` if any package develops a real build step). Verify with `pnpm clean && time pnpm typecheck` before/after. _(found tick 65, turbo.json:42)_
