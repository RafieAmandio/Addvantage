...
- [x] **Replace mock data** — plans, calendar, education, consult should hit real tables _(done tick 155 — all M1/M2/M3/M4 landed across ticks 141–149; parent flipped as bookkeeping.)_
  ...
  - [x] **M3.** `/app/plans` — blocked on Section 3 "Real plan authoring flow" (plan schema must land first) _(done tick 149 — unblocked by Section 3 P4; `/app/plan`, `/app/plan/[id]`, `/app/plan/archive`, `/app/plan/compare` are now server components reading `listPublishedPlans`/`getPlanById` and flowing rows through `dbPlanToTradingPlan` into the existing UI components. Empty-state for no-plans lands on `/app/plan`.)_
...
### Trading plan system
- [x] **Real plan authoring flow for admins (not mock)** — unblocks Section 2 M3 `/app/plans`. Carved into sub-items tick 145. _(done tick 155 — all P1/P2/P3/P4 landed across ticks 146–149; parent flipped as bookkeeping.)_
  - [x] **P1.** Schema — migration `0021_plans.sql`: `trading_plans` (id, symbol, thesis, direction, entry/stop/target numerics, r_multiple, tier, status draft|published|closed, author_id, created_at, published_at, closed_at), `plan_setups` (optional — keep as JSONB `setups` on plans for v1 to avoid table sprawl), RLS (public read where status='published', admin write). Reuse `public.is_admin()` helper. _(done tick 146 — chose single-table `trading_plans` with JSONB `setups` over a second `plan_setups` table for v1; keeps queries to one row per plan, lets us promote to a separate table later if indexed setup queries become a need. RLS splits admin writes into per-command policies (insert/update/delete) to avoid `multiple_permissive_policies` advisor. Tier enum reuses profile taxonomy (`free|vip`) from `0001_init` rather than inventing `tier-00/01`. Advisors: security clean, perf only INFO `unused_index` on fresh tables.)_
  - [x] **P2.** Feature query + types — `apps/web/features/plan/queries/plans.ts` (Zod-validated `listPublishedPlans`, `getPlanById`, `listMyDraftPlans` for admins), `features/plan/types.ts` with shared schemas. _(done tick 147 — chose to extend existing features/plan/types.ts with a canonical `PlanRowSchema` rather than create a separate db-row-types file; three named reads (listPublishedPlans, getPlanById, listMyDraftPlans) match the consumers queued in P3/P4 so no speculative exports.)_
  - [x] **P3.** Admin authoring UI — `/admin/plans` list + `/admin/plans/[id]` editor with server actions (create/update/publish/close), zod-validated, rate-limited, Sentry+logger. Reuse existing `features/plan/components/` shapes where they fit the new data model. _(done tick 148 — `/admin/plans` lists every status via new `listAllPlansForAdmin(100)` (admin-gated, RLS via `is_admin()`); `/admin/plans/new` + `/admin/plans/[id]` share `features/plan/components/admin/PlanEditorForm.tsx` driven by `useFormState` over `createPlan`/`updatePlan` bound actions; lifecycle buttons (`publishPlan`, `closePlan` with outcome select, `deletePlan`) live in the same form card. `features/plan/actions.ts` mirrors the `apps/web/app/admin/review/[id]/actions.ts` shape: `requireAdmin()` + `enforceAdminRateLimit(adminId, action)` at 30/60s + zod-validated inputs + `Sentry.captureException` + `logger.error` on every failure path + `revalidatePath('/admin/plans')` always, `/app/plans` on publish/close/delete. Inputs derive from `PlanRowSchema` via `PlanCreateInputSchema`/`PlanUpdateInputSchema` (with CSV→array, stringy-numeric→`number|null`, and `setups` JSON-array validators — no `as Type` on untrusted data). Setups ship as a JSON textarea for v1 (noted for future structured-rows promotion); pages stay thin (<20 LOC), all UI lives under `features/plan/components/admin/`. Admin nav gained a "Plans" link. `deletePlan` included — trivial and needed for correcting bad authorship rather than closing it.)_
  - [x] **P4.** Wire `/app/plans` (list) + `/app/plans/[id]` (detail) + `/app/plans/archive` + `/app/plans/compare` to the published-plans query; close Section 2 M3. _(done tick 149 — actual route is `/app/plan` (singular). Picked the adapter strategy (Plan → legacy TradingPlan via `features/plan/lib/adapt.ts`) over a UI rewrite because P4's charter is "wire to DB," not "redesign UI"; preserves every existing component and keeps the change to one tick. Adapter narrows tags to the closed Hashtag union via a `HASHTAGS.includes` guard, parses horizon from `horizon:*` tags (defaults to `swing`), pass-through on rich JSONB setups that match `TradingSetup` shape, else synthesizes one from the scalar entry/stop/target; per-setup adaptation try/guarded so a malformed JSONB entry can't nuke the render. Archive + compare bodies lifted into `features/plan/components/PlanArchive{,Compare}View.tsx` client islands so route files become thin async server components (<15 LOC each). Also closes M3.)_
- [x] **Setup outcome tracking (win/loss/R) tied to market close prices** — carved into sub-items tick 160. The `trading_plans` table (migration 0021) already carries `outcome` + `closed_at`; the admin `closePlan` action (tick 148) already sets outcome. What's missing: (a) the *price* the plan was closed at and the realized-R derived from it, (b) user-facing stats (win rate, avg R) on `/app/plan`, (c) optional automation that detects target/stop hits. _(done tick 180 — O1/O2/O3 landed across ticks 161–163; O4 explicitly deferred per note. Parent flipped as bookkeeping; the manual close flow is complete end-to-end.)_
  - [x] **O1.** Schema — migration `0023_plan_outcome_fields.sql`: add `trading_plans.close_price numeric`, `trading_plans.realized_r numeric` (both nullable — only set when `status='closed'`). Advisor pass (security + performance). No new RLS needed — columns inherit the existing row policy. Regenerate types. _(done tick 161 — additive nullable columns so no backfill or RLS policy edit needed; advisors: security clean, perf only INFO `unused_index` on the fresh indexes. Types regenerated; `Row`/`Insert`/`Update` for `trading_plans` now carry `close_price` + `realized_r`.)_
  - [x] **O2.** Wire admin close flow — `/admin/plans/[id]` `closePlan` action gains a `close_price` input; server-side compute `realized_r` from `(close_price - entry)/(entry - stop)` for longs, flipped for shorts, clamped to null when `entry`/`stop` missing. Persist both. Extend `PlanCloseInputSchema`. UI: add `close_price` numeric input next to the outcome select in `PlanEditorForm.tsx`. Validation + rate-limit inherited from the shared helper. _(done tick 162 — realized-R is derived from the row's own entry/stop (never client-trusted) and rounded to 2dp so the DB stores tidy values; also guards `entry === stop` for div-by-zero and fixed a stale `revalidatePath('/app/plans')` to the correct `/app/plan` route while in the area.)_
  - [x] **O3.** Stats surface — new `features/plan/queries/stats.ts` `getClosedPlanStats({ limit? })` returning `{ n, winRate, avgR, byDirection }`. Render a `PlanStatsBadges` component (feature-local) on `/app/plan` top-of-page. Stats are derived from `status='closed'` rows with non-null `realized_r`. Zod-validated. _(done tick 163 — aggregate derived in-process from an explicit-column select; schema lives in `features/plan/types.ts` so the client badge can import the type safely)_
  - [ ] **O4.** (Optional, future tick) Automated close detection — worker cron job `run:auto-close` that walks published plans, fetches the latest bar for each `symbol` via the bars adapter, and calls an internal close mutation when `target`/`stop` is hit. Deferred — admin-triggered close covers v1; only pull this forward if the manual close queue grows unwieldy.
...
### Consultation
- [x] **Real LLM-backed chat (replace canned keyword replies)** — carved into sub-items tick 150. _(done tick 155 — all L1/L2/L3/L4 landed across ticks 151–154; parent flipped as bookkeeping.)_
  - [x] **L1.** Non-streaming server action + system prompt — new `apps/web/lib/openai.ts` shared client (singleton, graceful no-op when `OPENAI_API_KEY` unset), server action `sendConsultMessage(sessionId, body)` in `features/consult/actions.ts` that (a) persists the user message via existing `appendConsultMessage`, (b) loads the session's recent message history (~20 turns), (c) calls `openai.chat.completions.create` with the desk system prompt + history, (d) persists the assistant reply, (e) returns the assistant message. Zod-validated input, rate-limited (10/60s per user), `logger.error` + `Sentry.captureException` on every error path. Graceful fallback to `pickReply` when `OPENAI_API_KEY` is unset. System prompt lives at `features/consult/lib/prompt.ts` (canonical desk persona — short, tactical, matches the existing `CANNED_REPLIES` tone; references primers by ID). _(done tick 151 — temperature 0.4 mirrors the worker's rephrase pipeline; empty-completion and LLM-exception paths both fall back to `pickReply` so the user's turn is never dropped (Sentry+logger still fire on the error path). History slice = last 20 messages from `listConsultMessages`. Hook wiring intentionally deferred to L2.)_
  - [x] **L2.** Wire `useConsultActions` to the action — replace the `pickReply(...)` call path in `features/consult/hooks/useConsultActions.ts` with `sendConsultMessage`. Preserve the typing-indicator UX (server-action roundtrip plays the same role). Keep `CANNED_REPLIES`/`pickReply` exported for offline fallback and for tests; no deletions. _(done tick 152 — persistable sessions go through `sendConsultMessage` (server-side LLM + DB persist in one roundtrip, eliminating the racy double-append); mock/desk read-only sessions keep the canned `setTimeout + pickReply` UX so the demo stays interactive offline. Typing indicator now spans the real network roundtrip. Toast on rate_limited / network error.)_
  - [x] **L3.** Streaming response — promote L1 to a streaming route handler `POST /api/consult/stream` using the OpenAI SDK's native stream (no Vercel AI SDK dep — fewer moving parts). Client reads via `ReadableStream` → progressively appends tokens into the assistant bubble. Server still persists the full final message post-stream. Edge runtime if possible; Node runtime if the Supabase client forces it. _(done tick 153 — chose nodejs runtime because getSession relies on next/headers cookies; plain-text streaming over SSE keeps the client a single ReadableStream reader with no framing library; LLM-error mid-stream swaps to pickReply so the user never sees a half-bubble)_
  - [x] **L4.** Token + cost observability — capture `usage.prompt_tokens` / `usage.completion_tokens` / `usage.total_tokens` from the OpenAI response, write to `consult_messages` (migration: add `metadata jsonb` column if not present, or new `tokens` column). `logger.info("consult.llm", { scope, sessionId, promptTokens, completionTokens, model })`. Emit a Sentry breadcrumb for the cost. Paves the way for Section 3 "Rate limit per tier" (tier-based daily token budget can key off this). _(done tick 154 — widened appendConsultMessage schema to carry metadata rather than doing a second UPDATE post-insert; metadata jsonb kept loose (optional per-field) to avoid schema churn when we add latency_ms / cached flags later; fallback paths record { model: "fallback.pickReply" } so usage dashboards can slice canned vs LLM turns.)_
...
- [x] **Rate limit per tier** — carved into sub-items tick 167. Today `lib/ratelimit.ts` applies a flat per-user bucket regardless of `profiles.tier` (free vs vip). This epic differentiates budgets by tier so VIP gets room to actually use the desk while free stays behind a cheap cap. L4's token-accounting metadata (tick 154) lets us layer a daily token budget on top of the existing per-minute rate. _(done tick 171 — all RT sub-items shipped)_
  - [x] **RT1.** Tier-aware limiter helper — new `apps/web/lib/ratelimit-tier.ts` wrapping `rateLimit({ key, limit, windowSec })` with a `(userId, tier, action)` signature. Per-tier bucket table (free = restrictive, vip = generous), resolved at call time. Graceful no-op when Upstash unset (inherits the existing lib contract). Single place to tweak tier budgets going forward. Exports both a named `enforceTierRateLimit()` helper and a `TIER_BUCKETS` const for test visibility. _(done tick 168 — helper + bucket table shipped; keyed `tier:<action>:<userId>` so retune of bucket values doesn't invalidate prior sliding-window state; action union constrains call sites to the documented set.)_
  - [x] **RT2.** Apply to consult — swap the flat 10/60s gate in `sendConsultMessage` (features/consult/actions.ts) and `/api/consult/stream/route.ts` for `enforceTierRateLimit(userId, tier, "consult:send")`. Tier is looked up via `getProfile()` inside the action so the wire only carries `userId`. Free: 5/60s. VIP: 30/60s. Anonymous is blocked by the existing auth gate. _(done tick 169 — tier resolved via getProfile() with narrow to "free"|"vip" and safe-default to "free" when profiles.tier is the looser `string` enum; debug log emitted on fallback so budget drift is observable.)_
  - [x] **RT3.** Daily consult token budget — new `features/consult/queries/usage.ts` `getDailyTokensUsed(userId)` summing `consult_messages.metadata->>'total_tokens'` over the last 24h (server-side; RLS restricts to caller's own rows anyway). `sendConsultMessage` + `/api/consult/stream` consult this before calling OpenAI; free tier capped at 10k tokens/day, VIP unbounded. Budget breach returns `rate_limited` with a dedicated reason so the UI can show "Daily token cap reached — upgrade for more" instead of the generic slow-down toast. _(done tick 170 — summed client-side in TS over last-24h rows to avoid a Postgres RPC for one aggregate; fail-open on query error so a broken metadata path doesn't DOS paying users; distinct `reason: "daily_token_cap"` threaded through both the action return shape and the stream 429 body so the toast copy can diverge from the per-minute throttle.)_
  - [x] **RT4.** Apply to /api/bars + /api/events — extend `enforceTierRateLimit` to cover anonymous callers (fall back to IP-keyed bucket when `userId` is missing — current behaviour). Authed callers get tier-based bucket. Tweaks the existing per-IP limiter without breaking unauth public reads. _(done tick 171 — union input `{ userId, tier, action } | { ip, action }`; authed key shape `tier:<action>:<userId>` kept stable so RT2 sliding windows don't reset, anon uses separate `tier:<action>:ip:<ip>` keyed to the free bucket. Routes resolve tier via `getProfile()` mirroring RT2; falls back to `x-forwarded-for`/`x-real-ip` for anon.)_
...
- [x] **Event → news correlation** — carved into sub-items tick 172. Today `timeline_events` (macro kind, from FF adapter) and `news_items` live side-by-side but have no link, so a user looking at a CPI release on the calendar can't see which news items covered it. Goal: surface the set of related news for any given calendar event, without hand-curation. _(done tick 180 — EN1/EN2 landed ticks 173–174; EN3 explicitly deferred (optional persistence). Parent flipped as bookkeeping; computed-at-read correlation is live on `/app/calendar/[id]`.)_
  - [x] **EN1.** Correlation query — new `apps/web/features/calendar/queries/correlation.ts` `listNewsForEvent(eventId)` returning published news whose `published_at` is within ±2h of the event's `occurred_at` AND shares ≥1 string in `affects[]` with the event. Zod-validated, bounded (`.limit(20)`), RLS-safe (uses `supabaseServer()`, inherits the `status='approved'` policy on news_items). No DB migration — pure JOIN on existing columns. _(done tick 173 — two-step query (event fetch then ±2h window + `news_items.affects` overlap on the event's `symbols[]`, since timeline_events exposes `symbols` not `affects`); reuses `NEWS_LIST_COLUMNS` / `NewsListRowSchema` / `toNewsListItem` from features/news (exported minimally) so the EN2 list UI can consume the same `NewsListItem` shape as /app/news; short-circuits to `[]` on missing event, empty symbols, or unparsable occurred_at.)_
  - [x] **EN2.** Surface on calendar detail — extend `/app/calendar/[id]` (or the event detail drawer/modal if that's the current shape) with a "Related news" section rendering a `RelatedNewsList` feature-local component (`features/calendar/components/`). Empty-state when none match. Link each row to `/app/news/[id]`. _(done tick 174 — new server route `/app/calendar/[id]` adds `getTimelineEventById` to `features/timeline/queries/timeline.ts` and reuses EN1's `listNewsForEvent`; `EventDetailView` + `RelatedNewsList` feature-local under `features/calendar/components/`; `EventRow` title now deeplinks to the detail page (`CalendarEvent.id` was already threaded through `timelineEventToCalendarEvent`). Empty state tells the user it's the ±2h window, not "no news today".)_
  - [ ] **EN3.** (Optional, deferred) Persist correlations — if the ±2h / affects heuristic needs tuning, add `timeline_events.related_news_ids uuid[]` + a worker job that backfills it. v1 stays computed-at-read (cheap, <20 rows, no cache needed).
...
- [x] **[reusability] `enforceAdminRateLimit` is duplicated verbatim across admin server-action files** — identical 14-line helper (rateLimit + logger.warn + throw "rate_limited") now lives in both `apps/web/app/admin/review/[id]/actions.ts` and `apps/web/features/plan/actions.ts`. Extract to `apps/web/lib/admin-ratelimit.ts` (next to `lib/ratelimit.ts`) with a configurable default of 30/60s and a `scope` arg for the `logger.warn` tag; both call sites become a one-line import. Future admin action files (P4 outcome tracking, consult admin moderation, education authoring) inherit the same limits for free. _(found tick 148, apps/web/features/plan/actions.ts:18-32 + apps/web/app/admin/review/[id]/actions.ts:13-27)_ _(done tick 156 — extracted to `apps/web/lib/admin-ratelimit.ts` with scope + optional limit/windowSec args; both files now import the shared helper and pass a per-file `ADMIN_SCOPE` const. 8 call sites converted (3 review + 5 plan); behavior unchanged.)_
- [x] **[code-quality] `/signup/profile/page.tsx` oversized (415 LOC)** — the profile wizard page inlines all 5 steps + local state machine. Break the steps into feature-local components under `features/signup/components/Step*.tsx` and lift the reducer into `features/signup/hooks/useProfileWizard.ts` so the route file thins back under 200 LOC. _(found tick 150, apps/web/app/(auth)/signup/profile/page.tsx)_ _(done tick 157 — route now <200 LOC; steps + hook live under features/signup/)_
...
- [x] **[observability] `useTimelineEvents` drops malformed realtime payloads via `console.warn` only** — the realtime INSERT subscription in `apps/web/features/timeline/hooks/useTimelineEvents.ts:71-73` silently skips rows that fail `TimelineEventSchema.safeParse`. If our DB writer drifts from the Zod schema (e.g. a new `kind` enum), live users stop seeing events on the chart and there's no server-side signal — the breakage lives only in browser devtools. Replace with `Sentry.captureMessage("useTimelineEvents: malformed payload", { level: "warning", extra: { issues: parsed.error.issues } })`; Sentry is already wired client-side per `sentry.client.config.ts`. _(found tick 156, apps/web/features/timeline/hooks/useTimelineEvents.ts:71)_ _(done tick 158 — swapped `console.warn` for `Sentry.captureMessage(…, { level: "warning", extra: { issues } })`; no-op when `SENTRY_DSN` unset, so dev behaviour unchanged.)_
- [x] **[performance] `listPublishedPrimers` is unpaginated** — `apps/web/features/education/queries/primers.ts:48` fetches every published primer row with no `.limit()` / `.range()`. Today the library is small (6 rows from the 0020 seed) so it's fine, but the query pattern fights the `listApprovedNews` / `listPublishedPlans` convention. Add a default `limit = 100` and `.range(0, limit-1)` so content-authored growth doesn't quietly bloat the page payload. _(found tick 164, apps/web/features/education/queries/primers.ts:48)_ _(done tick 166 — default limit=100 via `.range(0, limit-1)`; signature now takes optional `{ limit? }` to match the `listPublishedPlans` / `listApprovedNews` convention.)_
- [x] **[code-quality] `publishPlan` + `deletePlan` revalidate a 404 path** — both call `revalidatePath("/app/plans")` but the real user-facing route is `/app/plan` (singular — see `apps/web/app/app/plan/`). `closePlan` was fixed as a drive-by during tick 162; the other two still carry the stale path, so publishing or deleting a plan fails to invalidate the public feed's cache. One-line fix in each action. _(found tick 162, apps/web/features/plan/actions.ts publishPlan + deletePlan)_ _(done tick 165 — sed-swapped both paths to `/app/plan`.)_
- [x] **[performance] `getNewsItemById` uses `.select('*')` instead of an explicit column list** — every other read in `apps/web/features/news/queries/news.ts` narrows columns via a `NEWS_COLUMNS` / `NEWS_ADMIN_COLUMNS` constant (match `listPublishedPlans` pattern). Switching to an explicit list shields the route from accidental column bloat on the `news_items` table (`raw_text`, `metadata jsonb`, future diagnostic columns) and keeps the detail-page payload tight. _(found tick 160, apps/web/features/news/queries/news.ts:184)_ _(done tick 164 — added `NEWS_DETAIL_COLUMNS` constant (22 columns mirroring `NewsRowSchema`); swapped `.select('*')`.)_
- [x] **[reusability] 7 client components reimplement the same "state → URL searchParam" sync effect** — `useEffect` that reads/writes a handful of `useState` slices into `URLSearchParams` and calls `router.replace(qs ? \`${pathname}?${qs}\` : pathname, { scroll: false })`. Currently in `features/plan/components/PlanArchiveView.tsx`, `features/plan/components/PlanCompareView.tsx`, `features/education/components/EducationLibraryView.tsx`, `features/calendar/components/CalendarPageView.tsx`, `app/app/news/NewsListClient.tsx`, `features/consult/hooks/useSessionQueryParam.ts`, `app/app/tags/page.tsx`. Extract `useUrlSyncedState<T extends Record<string, string | null | undefined>>(params: T)` into `apps/web/lib/hooks/useUrlSyncedState.ts` that builds the qs and `router.replace`s on any change; optionally take an `{ scroll?: boolean }` arg. Shrinks each call site by ~8 lines and centralizes the "empty qs omits the `?`" + `scroll: false` defaults. _(found tick 149, apps/web/features/plan/components/PlanArchiveView.tsx:68-74 + 6 others)_ _(done tick 159 — one-way write-side hook at `apps/web/lib/hooks/useUrlSyncedState.ts`; 7 call sites migrated. Default mode builds a fresh URLSearchParams (6 sites); added `preserveExisting: true` option for `useSessionQueryParam` which needs to leave unrelated URL params intact. URL→state read on mount left in each call site's `useState` initializer.)_
- [x] **[performance] `listConsultMessages` is unpaginated** — `apps/web/features/consult/queries/messages.ts:52` returns every row for a session ordered ascending with no `.limit()`/`.range()`. A long-running desk session keeps growing, so this query's payload and the LLM history window both bloat over time. Add a default `limit = 200` trailing window (newest-N then reverse) with an override for admin export. _(found tick 175, apps/web/features/consult/queries/messages.ts:52)_ _(done tick 175 — signature gained `{ limit? }` with `DEFAULT_MESSAGE_LIMIT=200`; fetch newest-first + `.limit()` then `.slice().reverse()` so callers still receive chronological order. Three existing callers keep the default.)_
- [x] **[performance] `NewsListClient` renders inline per-row JSX without memoization** — `apps/web/app/app/news/NewsListClient.tsx:218` maps `filtered` news items into a large inline block (50+ lines per iteration, including nested `affects.map` and a `<Link>`). On each filter/search keystroke the whole list re-renders even when individual rows haven't changed. Extract the per-row block into a feature-local `NewsListRow` component under `features/news/components/`, wrap in `React.memo`, and pass narrow props; also extract to lift the file under 250 LOC. _(found tick 176, apps/web/app/app/news/NewsListClient.tsx:218)_ _(done tick 176 — extracted `features/news/components/NewsListRow.tsx` wrapped in `memo`; NewsListClient drops ~60 LOC, gets bailout on unchanged rows via referential equality of `item` (filtered slice preserves row refs when input unchanged).)_
- [x] **[performance] `EducationLibraryView` inline primer-card JSX is unmemoized** — `apps/web/features/education/components/EducationLibraryView.tsx:174` mirrors the NewsListClient pattern: ~55 LOC of JSX inline per primer, re-rendered on every search keystroke. Also the per-row `primers.findIndex((x) => x.id === p.id)` is O(n²). _(found tick 177, apps/web/features/education/components/EducationLibraryView.tsx:174)_ _(done tick 177 — extracted `features/education/components/PrimerCard.tsx` wrapped in `memo`; precomputed `indexById` map (O(1) lookup) replaces `findIndex`.)_
- [x] **[performance] Plan archive rows and month groups are unmemoized** — `PlanArchiveView.tsx` renders N `<PlanArchiveMonthGroup>` each containing M `<PlanArchiveRow>`; both re-render on every search keystroke because neither is memoized. Wrap both in `React.memo`. _(found tick 178)_ _(done tick 178 — both components now export a `memo()` wrapper; archive UI gets referential-equality bailout when `query`/`latestId`/`group` props match prior render.)_
- [x] **[observability] `useProfileWizard` uses `console.warn` for saveTraderProfile failures** — two client-side `console.warn` calls for server-action error + thrown exception (`apps/web/features/signup/hooks/useProfileWizard.ts:74,78`) leave no Sentry breadcrumb, so silent profile-persist failures are invisible in prod. Replace with `Sentry.captureMessage` / `Sentry.captureException` mirroring the tick-158 `useTimelineEvents` pattern. _(found tick 181, done tick 181 — `Sentry.captureMessage` for non-ok result + `Sentry.captureException` with `scope: "signup.saveTraderProfile"` tag for thrown path; no-op when SENTRY_DSN unset.)_
- [x] **[dx] worker declared `undici` as a dep but never imported it** — Node 20 has native `fetch`/`Request`/`Response` so the package was dead weight in `apps/worker/package.json` (≈1 MB installed + included in the Docker image). Verified no `from "undici"` / `require("undici")` anywhere in the repo. _(found and done tick 185 — removed the `undici` line from worker deps; `pnpm install` + `pnpm typecheck` green.)_
- [x] **[observability] `/api/consult/stream` swallows `req.json()` parse errors** — the body-parse `try { raw = await req.json(); } catch { return jsonError(400, "invalid_json"); }` at `apps/web/app/api/consult/stream/route.ts:102` returned 400 with no server-side signal. Parallel to the tick-126 Xendit `bad_json` treatment. _(found and done tick 184 — added `logger.warn("consult.stream: invalid JSON body", { scope, userId, error })` before returning; `logger` already imported.)_
