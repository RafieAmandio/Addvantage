...
- [ ] **[reusability] `enforceAdminRateLimit` is duplicated verbatim across admin server-action files** — identical 14-line helper (rateLimit + logger.warn + throw "rate_limited") now lives in both `apps/web/app/admin/review/[id]/actions.ts` and `apps/web/features/plan/actions.ts`. Extract to `apps/web/lib/admin-ratelimit.ts` (next to `lib/ratelimit.ts`) with a configurable default of 30/60s and a `scope` arg for the `logger.warn` tag; both call sites become a one-line import. Future admin action files (P4 outcome tracking, consult admin moderation, education authoring) inherit the same limits for free. _(found tick 148, apps/web/features/plan/actions.ts:18-32 + apps/web/app/admin/review/[id]/actions.ts:13-27)_
