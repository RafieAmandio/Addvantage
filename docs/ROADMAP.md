...
- [ ] **D5.** More macro sources — FOMC / CPI ingested into `timeline_events`. **Unblocked** as of tick 127 (ForexFactory adapter landed under Section 3 Calendar; `source_code='FF'` mirrors to `kind='macro'`).
  - [ ] **D5a.** Canonical `affects[]` mapping — FF adapter recognizes known high-impact indicator titles (FOMC rate decisions, CPI, NFP, PPI, retail sales, GDP, core PCE) and emits a stable `affects[]` array (e.g. FOMC → ['SPX','DXY','GOLD','BTC']; CPI → ['SPX','DXY','GOLD']) so the chart's kind='macro' markers actually filter to the relevant symbols.
  - [ ] **D5b.** Promotion of low-impact FF events into `news_items` with `status='rejected'` (or skip insert entirely) so the admin review queue isn't flooded with 30+ low-impact events per week.
