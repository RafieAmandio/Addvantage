...
- [ ] **Real LLM-backed chat (replace canned keyword replies)** — carved into sub-items tick 150.
  ...
  - [ ] **L2.** Wire `useConsultActions` to the action — replace the `pickReply(...)` call path in `features/consult/hooks/useConsultActions.ts` with `sendConsultMessage`. Preserve the typing-indicator UX (server-action roundtrip plays the same role). Keep `CANNED_REPLIES`/`pickReply` exported for offline fallback and for tests; no deletions.
