...
- [ ] **Rate limit per tier** — carved into sub-items tick 167. Today `lib/ratelimit.ts` applies a flat per-user bucket regardless of `profiles.tier` (free vs vip). This epic differentiates budgets by tier so VIP gets room to actually use the desk while free stays behind a cheap cap. L4's token-accounting metadata (tick 154) lets us layer a daily token budget on top of the existing per-minute rate.
  ...
  - [ ] **RT3.** Daily consult token budget — new `features/consult/queries/usage.ts` `getDailyTokensUsed(userId)` summing `consult_messages.metadata->>'total_tokens'` over the last 24h (server-side; RLS restricts to caller's own rows anyway). `sendConsultMessage` + `/api/consult/stream` consult this before calling OpenAI; free tier capped at 10k tokens/day, VIP unbounded. Budget breach returns `rate_limited` with a dedicated reason so the UI can show "Daily token cap reached — upgrade for more" instead of the generic slow-down toast.
